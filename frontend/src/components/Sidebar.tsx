...
export default function Sidebar() {
  ...
  return (
    <aside className="h-full flex flex-col items-center py-6 border-r z-50 overflow-visible bg-[var(--bg-sidebar)] border-[var(--border-color)] w-[80px]">
      <Link to="/context" className="mb-2 flex items-center justify-center font-extrabold text-xl cursor-pointer hover:scale-105 transition-transform w-12 h-12 rounded-xl bg-[var(--bg-panel)] text-[var(--accent-primary)] shadow-lg z-50" title="Workspace Hub">{logoText}</Link>
      ...
      <div className="flex-1 w-full flex flex-col items-center overflow-y-auto no-scrollbar space-y-2 pb-4">
          <Item to="/tasks" icon={Kanban} title="Task Board" id="tasks" /> {/* NEW LINK */}
      ...
      <div className="flex-col gap-4 mt-auto flex items-center pb-2">
          ...
          <button onClick={signOut} className="nav-item text-red-400 hover:text-red-500 hover:bg-red-500/10 group relative"><SignOut weight="bold" style={{ fontSize: '1.4rem' }} /></button>
      </div>
    </aside>
  );
}
