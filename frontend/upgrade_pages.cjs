...
const HIRING_DIR = path.join(__dirname, 'src', 'pages', 'Hiring');
const COMPONENTS_DIR = path.join(HIRING_DIR, 'components');

// Ensure directories exist
if (!fs.existsSync(HIRING_DIR)) fs.mkdirSync(HIRING_DIR, { recursive: true });
if (!fs.existsSync(COMPONENTS_DIR)) fs.mkdirSync(COMPONENTS_DIR, { recursive: true });

// --- 1. RE-WRITE HIRING INDEX (Main Logic Fixes) ---
...
fs.writeFileSync(path.join(HIRING_DIR, 'index.tsx'), MAIN_PAGE_CONTENT.trim());

console.log("✅ Hiring Page v6 Complete: Scroll logic fixed, Tooltip visible, Onboarding moved.");